...
export class VoteWrapper {
  ...
  fromBuffer(bytes: Buffer, offset: number = 0): number {
    ...
    offset += 2
    const voteTypeId = bintools.copyFrom(bytes, offset, offset + 4).readUInt32BE(0)
    offset += 4
    switch (voteTypeId) {
  ...
  toBuffer(): Buffer {
    ...
    return Buffer.concat([numLength, codecVersion, typeId, buff], numLength.length + totalByteLength)
  }
}
/**
...
export class AddVoteTx extends BaseTx {
  ...
  serialize(encoding: SerializedEncoding = "hex"): object {
    ...
    return {
      ...fields,
      voterAddress: serialization.encoder(this.voterAddress, encoding, "Buffer", "cb58"),
  ...
  toBuffer(): Buffer {
    ...
    let bsize: number = upgradeBuf.length + superbuff.length + this.proposalID.length + payloadBuffer.length
    const barr: Buffer[] = [
      upgradeBuf,
      superbuff,
      this.proposalID,
